...
class DataController
{
    ...
    private static function h($v){ return htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8'); }
    ...
    public static function contacts(){ self::requireLogin(); self::listTable(
        title: 'Contactos (página de contacto)',
        table: 'aaedi_contacts',
        cols: ['id','created_at','name','email','phone','reason_for_contact','message','ip','ua','status'],
        searchCols: ['name','email','phone','reason_for_contact','message','ip','ua'],
        viewRoute: '/admin/contacts/',
        exportRoute: '/admin/contacts/export'
    ); }

    public static function memberships(){ self::requireLogin(); self::listTable(
        title: 'Solicitudes de asociación',
        table: 'aaedi_membership_requests',
        cols: ['id','created_at','full_name','email','phone','firm','bar_number','city','message','consent_tos','consent_privacy','ip','ua','status'],
        searchCols: ['full_name','email','phone','firm','bar_number','city','message','ip','ua'],
        viewRoute: '/admin/memberships/',
        exportRoute: '/admin/memberships/export'
    ); }

    public static function memberContacts(){ self::requireLogin(); self::listTable(
        title: 'Contactos a miembros',
        table: 'aaedi_member_contacts',
        cols: ['id','created_at','member_slug','name','email','phone','message','ip','ua','status'],
        searchCols: ['member_slug','name','email','phone','message','ip','ua'],
        viewRoute: '/admin/member-contacts/',
        exportRoute: '/admin/member-contacts/export'
    ); }

    private static function listTable(string $title, string $table, array $cols, array $searchCols, string $viewRoute, string $exportRoute){
        $pdo = DB::conn();
        $q = trim($_GET['q'] ?? '');
        ...
        if ($q !== '') {
            $like = '%' . $q . '%';
            $parts = [];
            foreach($searchCols as $c){ $parts[] = "$c LIKE ?"; $params[] = $like; }
        ...
        echo '<thead><tr>';
        foreach($cols as $c) echo '<th>'.self::h($c).'</th>';
        ...
        foreach($rows as $r){
            ...
            foreach($cols as $c){
                $val = $r[$c] ?? '';
                if ($c==='message') $val = mb_strimwidth((string)$val, 0, 80, '…', 'UTF-8');
                echo '<td>'.self::h($val).'</td>';
            }
            echo '<td class="text-end"><a class="btn btn-sm btn-outline-primary" href="'.$viewRoute.(int)$r['id'].'">Ver</a></td>';
            echo '</tr>';
        }
        if (!$rows) echo '<tr><td colspan="'.(count($cols)+1).'" class="text-center text-muted">Sin resultados</td></tr>';
    ...
    public static function contact(int $id){ self::requireLogin(); self::showRecord('aaedi_contacts', $id, 'Contacto'); }
    public static function membership(int $id){ self::requireLogin(); self::showRecord('aaedi_membership_requests', $id, 'Solicitud de asociación'); }
    public static function memberContact(int $id){ self::requireLogin(); self::showRecord('aaedi_member_contacts', $id, 'Contacto a miembro'); }
    ...
    private static function showRecord(string $table, int $id, string $title){
        ...
        $st->execute([$id]);
        $row = $st->fetch();
        if (!$row){ http_response_code(404); self::layout($title, '<div class="alert alert-warning">No encontrado</div>'); return; }
    ...
    public static function exportContacts(){ self::requireLogin(); self::export('aaedi_contacts'); }
    public static function exportMemberships(){ self::requireLogin(); self::export('aaedi_membership_requests'); }
    public static function exportMemberContacts(){ self::requireLogin(); self::export('aaedi_member_contacts'); }
    ...
    private static function export(string $table){
        ...
        $where=''; $params=[];
        if ($q!==''){
            // busca en todas las columnas de texto (rápido: usa INFORMATION_SCHEMA solo una vez si quieres optimizar)
            $cols = $pdo->query("SHOW COLUMNS FROM $table")->fetchAll();
            $txtCols=[]; foreach($cols as $c){ $txtCols[] = $c['Field']; }
            $like='%'.$q.'%';
            $parts=[]; foreach($txtCols as $c){ $parts[]="$c LIKE ?"; $params[]=$like; }
            $where = 'WHERE '.implode(' OR ',$parts);
        }
        ...
        while($row = $st->fetch(\PDO::FETCH_ASSOC)){
            ...
            // normaliza saltos
            foreach ($row as &$v){ if (is_string($v)) $v = str_replace(["\r\n","\r"], "\n", $v); }
            fputcsv($out, $row);
        }
        fclose($out);
        exit;
